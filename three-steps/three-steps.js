/* ==========================================================================
   BUILD MARKER: v14-fill-fix-arrows — search for this exact string
...
(function () {
  ...
  function render(root) {
    ...
    root.innerHTML =
      ...
      '<div class="ts-track-arrow" style="top:33%"></div>' +
      '<div class="ts-track-arrow" style="top:66%"></div>' +
      stepsHtml +
  ...
  function initIntersectionEffect(root) {
    ...
    // --- Fill line: a SINGLE observer on the whole .ts-stepper container
    // (not per-step). An earlier version summed each step's individual
    // ratio, but that loses "credit" for a card the instant it scrolls
    // completely off-screen (its ratio drops to 0), capping how high the
    // sum could ever climb — which is why the fill looked stuck around
    // step 2 and never properly reached step 3 or the CTA. A single
    // observer on the full container doesn't have this problem: its
    // ratio increases smoothly and continuously as more of the (tall)
    // stepper scrolls past the top-half-biased line, all the way through.
    if (stepper) {
      var fillObserver = new IntersectionObserver(
        function (entries) {
          entries.forEach(function (entry) {
            if (!fill) return;
            fill.style.height = (entry.intersectionRatio * 100).toFixed(1) + "%";
          });
        },
        { threshold: THRESHOLDS, rootMargin: "0px 0px -50% 0px" }
      );
      fillObserver.observe(stepper);
    }
  }
