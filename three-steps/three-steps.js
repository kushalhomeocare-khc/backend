/* ==========================================================================
   BUILD MARKER: v20-fill-bug-arrows — search for this exact string
...
(function () {
  ...
  function render(root) {
    ...
    root.innerHTML =
      ...
      '<div class="ts-track-arrow ts-track-arrow-cta"></div>' +
      "<h3>" +
      DATA.cta.title +
      "</h3>" +
      "<p>" +
  ...
  function initIntersectionEffect(root) {
    ...
    // Reset trigger: purely monotonic peaks would stay solid forever
    // once reached, even after scrolling all the way back to the top.
    // An earlier fix used "all steps simultaneously invisible" as the
    // reset condition — but that's ALSO true the moment you scroll far
    // enough forward to reach the CTA (none of the 3 tracked step cards
    // are on-screen there either), which wiped progress right when it
    // should have been complete. Fixed by using a specific, unambiguous
    // signal instead: step 1 becoming visible again via an UNBIASED
    // observer (full viewport, no rootMargin) only happens on a genuine
    // backward scroll toward the top — it can never coincide with
    // viewing the CTA, which is far below step 1.
    if (steps[0]) {
  ...
  function positionGapArrows(root) {
    ...
    var gap1 = steps[1].offsetTop - 5;
    var gap2 = steps[2].offsetTop - 5;
