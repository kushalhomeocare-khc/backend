...
(function () {
  ...
  function buildFeeItem(tab, isOpen) {
    ...
    trigger.type = "button";
    trigger.className = "plan-trigger consult-fee-trigger";
    trigger.innerHTML =
      '<span class="consult-badge-large">' + (ICONS[tab.icon] || "") + '</span>' +
      '<span class="consult-region-label">' + tab.regionLabel + '</span>' +
      '<span class="consult-chev-btn">' + CHEV_ICON + '</span>';
    ...
    feeRow.className = "consult-fee-row2";
    feeRow.innerHTML =
      '<span class="plan-name">' + tab.heading + '</span>' +
      '<span class="plan-price">' + tab.price + '</span>';
    inner.appendChild(feeRow);
    ...
    if (tab.payment) {
      ...
      if (tab.payment.disclaimer) {
        payHtml += '<p class="ppn-disclaimer">' + tab.payment.disclaimer + '</p>';
      }
      payNote.innerHTML = payHtml;
  ...
  /* ---------- please note — fixed card, not collapsible ---------- */
  function buildNoteCard(tab) {
    var item = document.createElement("div");
    item.className = "plan-item consult-note-fixed";

    var header = document.createElement("div");
    header.className = "plan-trigger consult-note-header";
    header.innerHTML =
      '<div class="left">' +
        '<div class="consult-note-heading">' +
          tab.heading +
        '</div>' +
      '</div>';
    ...
    if (tab.outro) {
      ...
      po.className = "consult-note-intro";
      po.textContent = tab.outro;
  ...
  function wireAccordionItem(item, trigger, panel, inner) {
    trigger.addEventListener("click", function () {
      ...
      Array.prototype.forEach.call(stage.querySelectorAll(".plan-item"), function (sib) {
        if (sib !== item && !sib.classList.contains("consult-note-fixed")) {
          sib.classList.remove("open");
          var sibPanel = sib.querySelector(".plan-panel");
  ...
  fetch(DATA_URL)
    ...
    .then(function (data) {
      ...
      stage.appendChild(buildNoteCard(data.note));
      reportHeightSoon();
    })
    .catch(function (err) {
