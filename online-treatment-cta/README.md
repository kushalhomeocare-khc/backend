...
## About sideways scrolling on mobile only (not desktop)

This points to something specific about how GoDaddy's HTML section renders your code on phones: it runs inside its own iframe document (separate from your site's main `<head>`). If that inner document is missing a mobile viewport meta tag, phones default to rendering it at a wide desktop-style virtual viewport (~980px) and shrink it to fit — which causes sideways scroll/panning on mobile only, since desktop screens are already wide enough that the default doesn't matter there.

`online-treatment.js` now checks for a viewport meta tag and adds one if it's missing, and `online-treatment.css` adds an `overflow-x: hidden` safety net on `html, body` (safe here, since this stylesheet only loads inside GoDaddy's dedicated iframe for this section, not your whole site).

**Being upfront:** I can't see GoDaddy's actual iframe markup to confirm this is the exact cause — it's the most likely explanation given the mobile-only symptom, and the fix is a reasonable, low-risk one to try. Re-upload the updated CSS and JS, then test on mobile again. If the scrolling persists, it would help to know the exact phone/browser you're testing on, and whether it happens on every page load or only sometimes.
...
.ot-section *{ box-sizing: border-box; margin:0; padding:0; -webkit-tap-highlight-color: transparent; -webkit-touch-callout: none; }
html, body{
  overflow-x: hidden;
  max-width: 100%;
}
