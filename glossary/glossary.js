...
(() => {
  ...
  // Turn "Main Name (Alt, Alt2)" into individually-searchable lowercase
  // terms — every whole phrase AND every individual word inside it.
  // e.g. "Lumbar Spondylitis (Back Pain)" becomes:
  //   ["lumbar spondylitis", "lumbar", "spondylitis", "back pain", "back", "pain"]
  // Matching (in runSearch) is "starts with", so typing "pain" or "spon"
  // finds this condition even though neither is the first word overall.
  const getSearchTerms = (name) => {
    const match = name.match(/^(.*?)\s*\(([^)]+)\)\s*$/);
    const phrases = [];

    if (!match) {
      phrases.push(name.trim().toLowerCase());
    } else {
  ...
  const wireEvents = () => {
    ...
    els.azGrid.addEventListener('click', (e) => {
      ...
      // If a search is active, clear it first — otherwise items hidden
      // by the previous search filter would stay hidden inside the
      // letter card that's about to open.
      if (els.searchInput.value.trim() !== '') {
        els.searchInput.value = '';
        runSearch();
      }

      // Re-expand the alphabet grid if it was left collapsed (e.g. from
      // a previous search-box focus), so all letters are visible again.
      if (els.azBlock.classList.contains('collapsed')) {
        setAzCollapsed(false);
      }
  ...
  const reportHeight = () => {
    ...
    // Measure the actual content wrapper, not document.documentElement.
    // documentElement.scrollHeight reports the LARGER of "real content
    // height" or "current viewport height" — so once the iframe has been
    // tall once, it can only ever grow from there and never shrink back
    // down when a shorter letter/state is shown. Measuring the content
    // element itself (.az-section) gives the true height every time.
    const content = document.querySelector('.az-section') || document.body;
