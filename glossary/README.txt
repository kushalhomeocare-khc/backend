...
The glossary automatically reports its content height to the parent
page (this is built into glossary.js — see CONFIG.reportHeightToParent).
To make the iframe resize itself to match, use this embed snippet
on https://kushalonline.com/a-z-conditions (not just a plain <iframe>
tag on its own — the small script below is required for the
auto-resize to work):

    <iframe
      id="kushalGlossaryFrame"
    ...
    <script>
      window.addEventListener('message', function (event) {
        if (!event.data || event.data.type !== 'kushal-glossary-height') return;
        var frame = document.getElementById('kushalGlossaryFrame');
        if (frame) frame.style.height = event.data.height + 'px';
      });
    </script>

Why this matters:
  - Without this script, the iframe has no set height and browsers
    default it to a small box, OR (if you set a fixed min-height
    instead) you get either too much empty space when the glossary
    is folded, or an internal scrollbar when a letter/accordion
    makes it taller than the fixed height.
  - With this script, the iframe height updates automatically every
    time the glossary's content changes size — tapping a letter,
    opening a condition, searching, expanding/collapsing the A-Z
    grid, or resizing the window. No scrollbar, no dead space.
  - The glossary already has its own left/right spacing built in
    (2px on mobile), so the iframe itself can safely be set to 100%
    width with no extra padding needed on the parent page's side.
  - If you ever need to turn this off (e.g. embedding this page
    somewhere that isn't an iframe, or debugging), set
    reportHeightToParent: false in the CONFIG block at the top of
    glossary.js.
...
other code:
    ...
    reportHeightToParent    -> true/false — whether to tell the parent
                               page the content height for iframe
                               auto-resize (see section 3 above)

Also note: every treatment page link opens with target="_top", so
clicking one replaces the whole browser tab instead of loading the
treatment page nested inside the glossary's iframe.
