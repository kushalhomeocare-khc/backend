...
(function () {
  ...
  var ICONS = {
    ...
    global:
      '<svg viewBox="0 0 100 100">' +
        '<defs>' +
          '<radialGradient id="kushalGlobeGrad" cx="34%" cy="28%" r="80%">' +
            '<stop offset="0%" stop-color="#8FD8FA"/>' +
            '<stop offset="52%" stop-color="#2E8FC0"/>' +
            '<stop offset="100%" stop-color="#123047"/>' +
          '</radialGradient>' +
          '<radialGradient id="kushalGlobeSheen" cx="32%" cy="24%" r="45%">' +
            '<stop offset="0%" stop-color="#ffffff" stop-opacity="0.55"/>' +
            '<stop offset="100%" stop-color="#ffffff" stop-opacity="0"/>' +
          '</radialGradient>' +
          '<radialGradient id="kushalGlobeShade" cx="50%" cy="50%" r="65%">' +
            '<stop offset="68%" stop-color="#000000" stop-opacity="0"/>' +
            '<stop offset="100%" stop-color="#000000" stop-opacity="0.22"/>' +
          '</radialGradient>' +
          '<clipPath id="kushalGlobeClip"><circle cx="50" cy="50" r="48"/></clipPath>' +
        '</defs>' +
        '<g clip-path="url(#kushalGlobeClip)">' +
          ...
          '</g>' +
          '<circle cx="50" cy="50" r="48" fill="url(#kushalGlobeShade)"/>' +
          '<circle cx="50" cy="50" r="48" fill="url(#kushalGlobeSheen)"/>' +
        '</g>' +
        '<circle cx="50" cy="50" r="46.5" fill="none" stroke="#F0B429" stroke-width="1.3" opacity="0.6"/>' +
      '</svg>'
  };
  ...
  function renderRegion(regionKey) {
    ...
    // NRI pricing — accordion, folded by default (currently only present for Global)
    if (region.nriPricing) {
      ...
      var nriTrigger = document.createElement("button");
      nriTrigger.type = "button";
      nriTrigger.className = "nri-trigger";
      nriTrigger.innerHTML =
        '<span class="nri-trigger-title">' + nri.heading + '</span>' +
        '<svg class="nri-chev" viewBox="0 0 20 20" fill="none">' +
          '<path d="M5 7.5L10 12.5L15 7.5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>' +
        '</svg>';

      var nriPanel = document.createElement("div");
      nriPanel.className = "nri-panel";

      var nriInner = document.createElement("div");
      nriInner.className = "nri-panel-inner";
      ...
      nriInner.innerHTML =
        ...
        '<a href="' + (nri.whatsappUrl || "#") + '" target="_top" rel="noopener" class="plan-cta">' + nri.ctaLabel + '</a>';

      nriPanel.appendChild(nriInner);

      nriTrigger.addEventListener("click", function () {
        var willOpen = !nriWrap.classList.contains("open");
        nriWrap.classList.toggle("open", willOpen);
        nriPanel.style.maxHeight = willOpen ? nriPanel.scrollHeight + "px" : null;
        reportHeightSoon();
      });

      // keep an open panel's height correct if content reflows (font swap, etc.)
      if (window.ResizeObserver) {
        var nriRo = new ResizeObserver(function () {
          if (nriWrap.classList.contains("open")) {
            nriPanel.style.maxHeight = nriPanel.scrollHeight + "px";
          }
        });
        nriRo.observe(nriInner);
      }

      nriWrap.appendChild(nriTrigger);
      nriWrap.appendChild(nriPanel);
